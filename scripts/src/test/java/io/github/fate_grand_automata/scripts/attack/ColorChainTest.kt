...
class ColorChainTest {
    ...
    fun `Standard - lineup1 (1SB,2KQ,3NA,4NA,5SQ)`() {
        ...
        val values = cardTypeList.map { type ->
            colorChain.pick(
                cards = cards,
                cardType = type,
            )?.map { it.card } ?: emptyList()
        }
        val results = cardTypeList.zip(values).toMap()

        assertThat(
            results.getOrElse(CardTypeEnum.Buster) { emptyList() },
            "Buster"
        ).isEmpty()
        assertThat(
            results.getOrElse(CardTypeEnum.Arts) { emptyList() },
            "Arts"
        ).isEmpty()
        assertThat(
