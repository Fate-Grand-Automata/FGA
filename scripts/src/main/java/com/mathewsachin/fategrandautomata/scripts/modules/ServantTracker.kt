...
class ServantTracker(
    ...
    private val checkImages = mutableMapOf<TeamSlot, IPattern>()
    ...
    private fun init(teamSlot: TeamSlot, slot: ServantSlot) {
        ...
        checkImages[teamSlot] = game.servantChangeCheckRegion(slot).getPattern()
    ...
    private fun check(slot: ServantSlot) {
        ...
        checkImages[teamSlot].let { img ->
            if (img == null) {
                init(teamSlot, slot)
            }
            else if (
                img !in game.servantChangeCheckRegion(slot)
